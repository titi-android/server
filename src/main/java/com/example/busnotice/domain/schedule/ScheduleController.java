...
public class ScheduleController {
    ...
    @GetMapping("/v3/schedules/today")
    @Operation(
            summary = "오늘 모든 스케줄의 교통편 정보 조회",
    ...
    public ApiResponse<List<ScheduleArrivalResponse>> getAllSchedulesOfTodayV2(
        ...
        List<ScheduleArrivalResponse> schedulesWithArrivals = scheduleService.특정_요일의_스케줄들의_교통편_정보(userDetails.getId(), DayConverter.getTodayAsString());
    ...
    @GetMapping("/v3/schedules/days")
    @Operation(
            summary = "특정 요일의 모든 스케줄의 교통편 정보 조회",
            description = "오늘 스케줄이 없는 경우 빈 리스트를 반환"
    )
    public ApiResponse<List<ScheduleArrivalResponse>> getAllSchedulesOfDaysV2(
            @RequestParam("days") String days,
            @AuthenticationPrincipal CustomUserDetails userDetails
    ) throws UnsupportedEncodingException {
        List<ScheduleArrivalResponse> scheduleResponses = scheduleService.특정_요일의_스케줄들의_교통편_정보(
    ...
    @GetMapping("/v3/schedules/now")
    @Operation(
            summary = "현재 스케줄의 교통편 정보 조회",
            description = "현재 스케줄이 없는 경우 null 을 반환"
    )
    public ApiResponse<ScheduleArrivalResponse> getCurrentScheduleV2(
            @AuthenticationPrincipal CustomUserDetails userDetails
    ) throws UnsupportedEncodingException {
        ScheduleArrivalResponse currentScheduleWithArrivals = scheduleService.현재_스케줄의_교통편_정보(userDetails.getId());
        String msg = (currentScheduleWithArrivals == null)
                ? "현재 스케줄이 존재하지 않습니다."
                : "현재 스케줄이 존재합니다.";
        return ApiResponse.createSuccessWithData(currentScheduleWithArrivals, msg);
    }
}
